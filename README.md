...
├── results/
│   ├── benchmark_consistent_v4.csv             # generated by Stage 05; primary benchmark table
│   ├── fig2_combined_cache.json                # pre-computed tAI/CSI cache
│   ├── figS_tai_other_orgs_cache.json          # pre-computed tAI cache (other organisms)
│   ├── figS_csi_all39_cache.json               # pre-computed CSI cache (all 39 organisms)
│   └── OLD_RESULTS/
│       ├── large_benchmark_combined.csv        # extended benchmark (489 proteins)
│       └── native_cpb_100genes.csv             # native CPB baseline
└── other_optimizers/
    ├── benchmark/
...
**Run-order note:** `results/benchmark_consistent_v4.csv` is written by Stage 05
and is also included pre-computed in the data deposit. All other pre-computed
files in `results/` are included in the data deposit and do not need to be
regenerated.
