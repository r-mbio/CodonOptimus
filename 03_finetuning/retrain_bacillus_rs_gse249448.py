...
OUT     = BASE / 'models/industrial_mlm_all39_rs_bacillus.pt'
BACKUP  = BASE / 'models/old_models/industrial_mlm_all39_rs_bacillus_gse126234.pt'
...
# ── Load GSE249448 WT0 expression ──────────────────────────────────────────────
print('\nLoading GSE249448 WT0 Ribo-seq TPM...')
...
print(f'  Genes: {len(df_gse)}  TPM range: {df_gse.wt0_mean.min():.1f}–{df_gse.wt0_mean.max():.0f}')

# ── Load training CSV and join ─────────────────────────────────────────────────
...
print(f'  Matched training genes: {len(df)} / 3067')

# Log transform + top-10%
df['log_tpm'] = np.log1p(df['wt0_mean'])
thresh  = df['log_tpm'].quantile(1 - TOP_PCT/100)
df_hi   = df[df['log_tpm'] >= thresh]
print(f'  Top-{TOP_PCT}% TPM threshold: {thresh:.2f} log  ({df_hi.wt0_mean.mean():.0f} TPM mean)')
...
print(f'  Batches/epoch: {len(loader)}  Epochs: {EPOCHS}')
...
for epoch in range(1, EPOCHS+1):
    ...
    avg=total_loss/max(n_tok,1); ppl=math.exp(min(avg,20))
    lr_now=scheduler.get_last_lr()[0]
    print(f'Epoch {epoch:3d}/{EPOCHS}  loss={avg:.4f}  ppl={ppl:.3f}  lr={lr_now:.2e}')
    if avg<best_loss:
        best_loss=avg
        torch.save({'model':model.state_dict(),'epoch':epoch,
                    'source':'GSE249448_WT0','n_genes':len(samples)}, OUT)

print(f'\nBest loss: {best_loss:.4f} → {OUT}')
